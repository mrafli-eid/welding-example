...
export class DatefilterInterceptor implements HttpInterceptor {
    ...
    intercept(
        ...
        if (
            ...
            request.params.get('start') !== 'null'
        ) {
            let start: Date = new Date(request.params.get('start'));
            modifiedParams = modifiedParams.set(
                'start',
                this.toIsoString(start)
            );
        }
        ...
        if (
            request.params.get('act_date') &&
            request.params.get('act_date') !== 'null'
        ) {
            let act_date: Date = new Date(request.params.get('act_date'));
            modifiedParams = modifiedParams.set(
                'act_date',
                this.toIsoString(act_date)
            );
        }

        if (
            request.params.get('plan_date') &&
            request.params.get('plan_date') !== 'null'
        ) {
            let plan_date: Date = new Date(request.params.get('plan_date'));
            modifiedParams = modifiedParams.set(
                'plan_date',
                this.toIsoString(plan_date)
            );
        }
