...
import { MasterParams, MasterSubject } from '../../../../core/models/master.model';
...
export class MasterSubjectListComponent {
    ...
    getMasterList() {
        ...
        this.masterService.getSubjectList(this.queryParams)
            .pipe(take(1))
            .subscribe({
                next: (response) => {
    ...
    delete(masterSubject: MasterSubject) {
        ...
        matDialogRef.afterClosed().subscribe((resp) => {
            if (resp) {
                this.masterService.deleteSubject(masterSubject.id)
                    .pipe(take(1))
                    .subscribe(() => {
                        this.pagination.page_number = 1;
                        this.getMasterList();
