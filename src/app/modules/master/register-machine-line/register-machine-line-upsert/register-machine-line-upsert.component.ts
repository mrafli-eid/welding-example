...
export class RegisterMachineLineUpsertComponent {
    ...
    ngOnInit() {
        ...
        this.notSelectedFormControl.valueChanges.subscribe((text) => {
            this.filteredNotSelectedMachineList = this.notSelectedMachineList.filter((m) => m.name.toLowerCase().includes(text.toLowerCase()));
        });
        this.selectedFormControl.valueChanges.subscribe((text) => {
            this.filteredSelectedMachineList = this.selectedMachineList.filter((m) => m.name.toLowerCase().includes(text.toLowerCase()));
        });
    ...
    selectOneMachine(machine: MasterMachineLine) {
        ...
        this.notSelectedMachineList.find(m => m.id == machine.id).selected = machine.selected;
    }

    unselectOneMachine(machine: MasterMachineLine) {
        machine.selected = !machine.selected;
        this.selectedMachineList.find(m => m.id == machine.id).selected = machine.selected;
    }
    ...
    unselectBulk() {
        ...
        selected.forEach((m) => {
            ...
            const index = this.selectedMachineList.findIndex(o => o.id === m.id);
    ...
    selectBulk() {
        ...
        selected.forEach((m) => {
            ...
            const index = this.notSelectedMachineList.findIndex(o => o.id === m.id);
    ...
    refreshData() {
        this.filteredNotSelectedMachineList = this.notSelectedMachineList.filter((m) =>
    ...
    patchValue() {
        if (this.masterData) {
            ...
            this.masterData.machines.forEach((m) => {
                ...
                const machine = this.notSelectedMachineList.find(o => o.id === m.machine_id);
                if (machine) {
