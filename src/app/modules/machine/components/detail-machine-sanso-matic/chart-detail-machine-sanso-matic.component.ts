...
export class ChartDetailMachineSansoMaticComponent {
  ...
  ngOnChanges() {
    this.lineChartData.datasets[0].data = this.data.data_sanso_matic_a.map((d) => d.value);
    this.lineChartData.datasets[1].data = this.data.data_sanso_matic_b.map((d) => d.value);
